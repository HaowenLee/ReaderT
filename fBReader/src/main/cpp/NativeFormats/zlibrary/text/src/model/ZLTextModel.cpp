...
ZLTextModel::ZLTextModel(const std::string &id, const std::string &language,
...
ZLTextModel::~ZLTextModel() {
    for (std::vector<ZLTextParagraph *>::const_iterator it = myParagraphs.begin();
         it != myParagraphs.end(); ++it) {
        delete *it;
    }
}
...
ZLTextPlainModel::ZLTextPlainModel(const std::string &id, const std::string &language,
                                   ...
                                   const std::string &fileExtension, FontManager &fontManager) :
...
void ZLTextPlainModel::createParagraph(ZLTextParagraph::Kind kind) {
    ZLTextParagraph *paragraph = (kind == ZLTextParagraph::TEXT_PARAGRAPH) ? new ZLTextParagraph()
                                                                           : new ZLTextSpecialParagraph(
                    kind);
...
*/
    ...
    *address++ = depth;
    ...
    if (entry.isFeatureSupported(ZLTextStyleEntry::COLOR)) {
        *address++ = entry.myColor[1];
        *address++ = entry.myColor[2];
        *address++ = entry.myColor[3];
        *address++ = entry.myColor[4];
        *address++ = entry.myColor[5];
        *address++ = entry.myColor[6];
    }
    // --- writing entry
...
void ZLTextModel::addHyperlinkControl(ZLTextKind textKind, ZLHyperlinkType hyperlinkType,
...
void ZLTextModel::addVideoEntry(const ZLVideoEntry &entry) {
    ...
    std::size_t len = 4;
    for (std::map<std::string, std::string>::const_iterator it = sources.begin();
         it != sources.end(); ++it) {
        len += 2 * (ZLUnicodeUtil::utf8Length(it->first) + ZLUnicodeUtil::utf8Length(it->second)) +
               4;
    }
...
void ZLTextModel::addExtensionEntry(const std::string &action,
