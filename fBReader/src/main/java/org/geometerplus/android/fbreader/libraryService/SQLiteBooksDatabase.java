...
final class SQLiteBooksDatabase extends BooksDatabase {
    ...
    private void updateTables24() {
        myDatabase.execSQL("INSERT OR REPLACE INTO HighlightingStyle (style_id, name, bg_color) VALUES (1, '', 217*256*256 + 237*256 + 249)"); // #888a85
    ...
    protected List<Bookmark> loadBookmarks(BookmarkQuery query) {
        ...
        sql
                ...
                .append(" AND ")
                .append(" bm.mark_type = " + query.MarkType)
    ...
    protected long saveBookmark(Bookmark bookmark) {
        ...
        System.out.println(" 书签类型 3 ==> " + bookmark.MarkType);

        if (bookmarkId == -1) {
            statement = get(
                    "INSERT INTO Bookmarks (uid,version_uid,book_id,bookmark_text,original_text,creation_time,modification_time,access_time,model_id,paragraph,word,char,end_paragraph,end_word,end_character,visible,mark_type,style_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
            );
        } else {
            statement = get(
                    "UPDATE Bookmarks SET uid=?,version_uid=?,book_id=?,bookmark_text=?,original_text=?,creation_time=?,modification_time=?,access_time=?,model_id=?,paragraph=?,word=?,char=?,end_paragraph=?,end_word=?,end_character=?,visible=?,mark_type=?,style_id=? WHERE bookmark_id=?"
            );
        }
        ...
        synchronized (statement) {
            ...
            statement.bindLong(++fieldCount, bookmark.MarkType);
